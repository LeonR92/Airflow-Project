...
def transformation():
    df = pd.read_csv("/opt/airflow/dags/raw.csv")

    # Schema correction
    df['tpep_pickup_datetime'] = pd.to_datetime(df['tpep_pickup_datetime'])
    df['tpep_dropoff_datetime'] = pd.to_datetime(df['tpep_dropoff_datetime'])

    # Duplicates, if any, are dropped
    ...
    # Export each dimension table to CSV files
    datetime_dim.to_csv('/opt/airflow/dags/datetime_dim.csv', index=False)
    passenger_count_dim.to_csv('/opt/airflow/dags/passenger_count_dim.csv', index=False)
    trip_distance_dim.to_csv('/opt/airflow/dags/trip_distance_dim.csv', index=False)
    rate_code_dim.to_csv('/opt/airflow/dags/rate_code_dim.csv', index=False)
    pickup_location_dim.to_csv('/opt/airflow/dags/pickup_location_dim.csv', index=False)
    dropoff_location_dim.to_csv('/opt/airflow/dags/dropoff_location_dim.csv', index=False)
    payment_type_dim.to_csv('/opt/airflow/dags/payment_type_dim.csv', index=False)
    ...
    fact_table.to_csv('/opt/airflow/dags/fact_table.csv', index=False)
    
    print(datetime_dim.head(5))
